...
func (s *apiService) Start() {
	s.logger.Info().Msg("commit-tracker server is starting up")
	...
	err := g.Wait()
	if err != nil {
		s.logger.Info().Msgf("error occured starting commit-tracker server: %s", err.Error())
	}
	s.logger.Info().Msg("commit-tracker server shutting down...")
}
...
func (s *apiService) Serve(ctx context.Context) error {
  ...
  loggedMux := s.logMiddleware(router)
	...
	go func() {
		s.logger.Info().Msgf("starting commit-tracker server, address: %s", s.addr)
	...
	select {
	case <-ctx.Done():
		s.logger.Info().Msg("commit-tracker server graceful shutdown started")
		server.Shutdown(ctx)
		s.logger.Info().Msg("commit-tracker server successfully shutdown")
		return nil
	case err := <-errChan:
		s.logger.Info().Msgf("commit-tracker server shutdown with error: %s", err.Error())
		return err
	}
}
